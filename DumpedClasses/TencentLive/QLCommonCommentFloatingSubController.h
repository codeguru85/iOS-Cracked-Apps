//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "QLMomentTableViewController.h"

#import "P_QLDragToRefresh.h"
#import "QLMomentTimelineViewDelegate.h"

@class NSMutableArray, NSString, QLMomentExposureItem, UIView;

@interface QLCommonCommentFloatingSubController : QLMomentTableViewController <QLMomentTimelineViewDelegate, P_QLDragToRefresh>
{
    _Bool _canLoadMore;
    NSString *_dataKey;
    NSString *_pageTitle;
    NSString *_localVid;
    NSMutableArray *_mergedItemList;
    UIView *_commentView;
    QLMomentExposureItem *_exposureItem;
}

@property(retain, nonatomic) QLMomentExposureItem *exposureItem; // @synthesize exposureItem=_exposureItem;
@property(retain, nonatomic) UIView *commentView; // @synthesize commentView=_commentView;
@property(nonatomic) _Bool canLoadMore; // @synthesize canLoadMore=_canLoadMore;
@property(retain, nonatomic) NSMutableArray *mergedItemList; // @synthesize mergedItemList=_mergedItemList;
@property(copy, nonatomic) NSString *localVid; // @synthesize localVid=_localVid;
@property(copy, nonatomic) NSString *pageTitle; // @synthesize pageTitle=_pageTitle;
@property(copy, nonatomic) NSString *dataKey; // @synthesize dataKey=_dataKey;
- (void).cxx_destruct;
- (void)deleteMomentItemNotification:(id)arg1;
- (void)needToSeeMoreComment:(id)arg1 withItem:(id)arg2;
- (void)needToUpdateCommentView:(id)arg1 withItem:(id)arg2;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (double)tableView:(id)arg1 heightForRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (id)tableView:(id)arg1 viewForHeaderInSection:(long long)arg2;
- (double)tableView:(id)arg1 heightForHeaderInSection:(long long)arg2;
- (void)dragReload:(_Bool)arg1;
- (void)configureFooterDragView;
- (_Bool)hasFootterDragRefresh;
- (_Bool)hasHeaderDragRefresh;
- (id)dragRefreshView;
- (void)scrollViewDidScroll:(id)arg1;
- (void)mergeFakeItemToTimeLine;
- (void)deleteMomentCell:(id)arg1;
- (void)reloadMomentCell:(id)arg1 needSetContentOffset:(_Bool)arg2 animated:(_Bool)arg3;
- (void)updateMomentCell:(id)arg1;
- (void)handlePutCommentFailed:(id)arg1;
- (void)handlePutCommentSucess:(id)arg1;
- (void)handlePublishFailed:(id)arg1;
- (void)handlePictureFailed:(id)arg1;
- (void)handlePublishSucess:(id)arg1;
- (id)indexPathWithObject:(id)arg1;
- (void)engineDidFailed:(id)arg1;
- (void)engineDidFinishLoad:(id)arg1;
- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void *)arg4;
- (id)titleForEmpty;
- (struct CGRect)frameOfPageStatusView;
- (void)addPublishNotification;
- (_Bool)shouldShowBtttomStarStarCommentView;
- (void)shareCommentView:(id)arg1;
- (void)loadDataWithCachePolicy:(int)arg1;
- (void)viewWillLayoutSubviews;
- (_Bool)shouldIgnorePageForNaviReport;
- (void)viewDidDisappear:(_Bool)arg1;
- (void)viewWillDisappear:(_Bool)arg1;
- (void)viewDidAppear:(_Bool)arg1;
- (void)viewWillAppear:(_Bool)arg1;
- (void)dealloc;
- (void)viewDidLoad;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2;
- (id)initWithDataKey:(id)arg1 pageTitle:(id)arg2 localVid:(id)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

