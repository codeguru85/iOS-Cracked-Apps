//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "QLRequestModel.h"

@class MoabCache, QLASIHTTPRequest, QLJCEUserCenterVipEntryResponse;

@interface QLUserCenterHollywoodEntryModel : QLRequestModel
{
    MoabCache *_cahce;
    QLJCEUserCenterVipEntryResponse *_response;
    QLASIHTTPRequest *_dataRequest;
}

@property(retain, nonatomic) QLASIHTTPRequest *dataRequest; // @synthesize dataRequest=_dataRequest;
@property(retain, nonatomic) QLJCEUserCenterVipEntryResponse *response; // @synthesize response=_response;
- (void).cxx_destruct;
- (void)cancelDataRequest;
- (void)processResponce:(id)arg1 isCache:(_Bool)arg2;
- (void)requestDidRetryRequest:(id)arg1;
- (void)request:(id)arg1 didFailLoadWithError:(id)arg2;
- (void)requestDidFinishLoad:(id)arg1;
- (void)load:(int)arg1 more:(_Bool)arg2;
- (void)dealloc;
- (id)init;

@end

