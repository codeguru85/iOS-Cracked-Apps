//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "JceObjectV2.h"

@class NSString;

@interface QLJCEDynamicDataRequest : JceObjectV2
{
    NSString *jcev2_p_0_r_type;
    NSString *jcev2_p_1_r_dataKey;
    NSString *jcev2_p_2_o_pageContext;
    NSString *jcev2_p_3_o_vid;
    NSString *jcev2_p_4_o_referKey;
}

+ (id)jceType;
+ (void)initialize;
@property(retain, nonatomic, getter=jce_referKey, setter=setJce_referKey:) NSString *jcev2_p_4_o_referKey; // @synthesize jcev2_p_4_o_referKey;
@property(retain, nonatomic, getter=jce_vid, setter=setJce_vid:) NSString *jcev2_p_3_o_vid; // @synthesize jcev2_p_3_o_vid;
@property(retain, nonatomic, getter=jce_pageContext, setter=setJce_pageContext:) NSString *jcev2_p_2_o_pageContext; // @synthesize jcev2_p_2_o_pageContext;
@property(retain, nonatomic, getter=jce_dataKey, setter=setJce_dataKey:) NSString *jcev2_p_1_r_dataKey; // @synthesize jcev2_p_1_r_dataKey;
@property(retain, nonatomic, getter=jce_type, setter=setJce_type:) NSString *jcev2_p_0_r_type; // @synthesize jcev2_p_0_r_type;
- (void).cxx_destruct;
- (id)init;

@end

