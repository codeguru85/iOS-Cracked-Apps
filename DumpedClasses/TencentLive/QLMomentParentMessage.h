//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "QLMomentItemBase.h"

#import "NSCoding.h"

@class NSMutableArray, NSString, QLMomentUser;

@interface QLMomentParentMessage : QLMomentItemBase <NSCoding>
{
    NSString *_msgID;
    NSString *_content;
    long long _msgType;
    NSMutableArray *_imageList;
    QLMomentUser *_user;
}

+ (id)messageWithRootDict:(id)arg1;
+ (id)messageWithParentDict:(id)arg1;
@property(retain, nonatomic) QLMomentUser *user; // @synthesize user=_user;
@property(retain, nonatomic) NSMutableArray *imageList; // @synthesize imageList=_imageList;
@property(nonatomic) long long msgType; // @synthesize msgType=_msgType;
@property(copy, nonatomic) NSString *content; // @synthesize content=_content;
@property(copy, nonatomic) NSString *msgID; // @synthesize msgID=_msgID;
- (void).cxx_destruct;
- (id)initWithCoder:(id)arg1;
- (void)encodeWithCoder:(id)arg1;

@end

