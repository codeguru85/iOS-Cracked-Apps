//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UICollectionViewCell.h"

@class UIImageView, UILabel;

@interface XYPHExploreBoardCell : UICollectionViewCell
{
    UILabel *_nameLabel;
    UILabel *_countLabel;
    UIImageView *_imageView1;
    UIImageView *_imageView2;
    UIImageView *_imageView3;
    UIImageView *_imageView4;
    UIImageView *_avatarImageView;
    UILabel *_userHeadLabel;
    UILabel *_userLabel;
    UILabel *_userTailLabel;
    UIImageView *_PrivateIcon;
    UILabel *_illegalInfoLabel;
}

@property(retain, nonatomic) UILabel *illegalInfoLabel; // @synthesize illegalInfoLabel=_illegalInfoLabel;
@property(retain, nonatomic) UIImageView *PrivateIcon; // @synthesize PrivateIcon=_PrivateIcon;
@property(retain, nonatomic) UILabel *userTailLabel; // @synthesize userTailLabel=_userTailLabel;
@property(retain, nonatomic) UILabel *userLabel; // @synthesize userLabel=_userLabel;
@property(retain, nonatomic) UILabel *userHeadLabel; // @synthesize userHeadLabel=_userHeadLabel;
@property(retain, nonatomic) UIImageView *avatarImageView; // @synthesize avatarImageView=_avatarImageView;
@property(retain, nonatomic) UIImageView *imageView4; // @synthesize imageView4=_imageView4;
@property(retain, nonatomic) UIImageView *imageView3; // @synthesize imageView3=_imageView3;
@property(retain, nonatomic) UIImageView *imageView2; // @synthesize imageView2=_imageView2;
@property(retain, nonatomic) UIImageView *imageView1; // @synthesize imageView1=_imageView1;
@property(retain, nonatomic) UILabel *countLabel; // @synthesize countLabel=_countLabel;
@property(retain, nonatomic) UILabel *nameLabel; // @synthesize nameLabel=_nameLabel;
- (void).cxx_destruct;
- (void)prepareForReuse;
- (void)initialize;
- (id)initWithFrame:(struct CGRect)arg1;

@end

