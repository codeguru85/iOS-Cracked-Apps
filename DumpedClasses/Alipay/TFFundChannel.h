//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NSCoding.h"

@class NSString, TFBankCardChannel;

@interface TFFundChannel : NSObject <NSCoding>
{
    _Bool _needBindNewCard;
    NSString *_assignedChannel;
    NSString *_availableAmount;
    NSString *_channelName;
    NSString *_channelType;
    TFBankCardChannel *_bankCardChannel;
}

@property(retain, nonatomic) TFBankCardChannel *bankCardChannel; // @synthesize bankCardChannel=_bankCardChannel;
@property(retain, nonatomic) NSString *channelType; // @synthesize channelType=_channelType;
@property(retain, nonatomic) NSString *channelName; // @synthesize channelName=_channelName;
@property(nonatomic) _Bool needBindNewCard; // @synthesize needBindNewCard=_needBindNewCard;
@property(retain, nonatomic) NSString *availableAmount; // @synthesize availableAmount=_availableAmount;
@property(retain, nonatomic) NSString *assignedChannel; // @synthesize assignedChannel=_assignedChannel;
- (void).cxx_destruct;
@property(readonly, nonatomic) NSString *channelNameForMultiLanuage;
- (id)initWithCoder:(id)arg1;
- (void)encodeWithCoder:(id)arg1;

@end

