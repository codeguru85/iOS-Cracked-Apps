//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "APDPBGeneratedMessage.h"

@class NSString;

@interface LIFEAPPSubmitContentPraiseRespPB : APDPBGeneratedMessage
{
}

+ (CDStruct_af61540b *)_fieldInfos;

// Remaining properties
@property(retain, nonatomic) NSString *bizNo; // @dynamic bizNo;
@property(retain, nonatomic) NSString *bizType; // @dynamic bizType;
@property(retain, nonatomic) NSString *clientId; // @dynamic clientId;
@property(nonatomic) long long gmtCreate; // @dynamic gmtCreate;
@property(nonatomic) long long gmtModify; // @dynamic gmtModify;
@property(readonly) _Bool hasBizNo; // @dynamic hasBizNo;
@property(readonly) _Bool hasBizType; // @dynamic hasBizType;
@property(readonly) _Bool hasClientId; // @dynamic hasClientId;
@property(readonly) _Bool hasMemo; // @dynamic hasMemo;
@property(readonly) _Bool hasRecoverClientId; // @dynamic hasRecoverClientId;
@property(readonly) _Bool hasRecoverServerId; // @dynamic hasRecoverServerId;
@property(readonly) _Bool hasResultStatus; // @dynamic hasResultStatus;
@property(readonly) _Bool hasSceneCode; // @dynamic hasSceneCode;
@property(readonly) _Bool hasServerId; // @dynamic hasServerId;
@property(retain, nonatomic) NSString *memo; // @dynamic memo;
@property(retain, nonatomic) NSString *recoverClientId; // @dynamic recoverClientId;
@property(retain, nonatomic) NSString *recoverServerId; // @dynamic recoverServerId;
@property(retain, nonatomic) NSString *resultStatus; // @dynamic resultStatus;
@property(retain, nonatomic) NSString *sceneCode; // @dynamic sceneCode;
@property(retain, nonatomic) NSString *serverId; // @dynamic serverId;

@end

