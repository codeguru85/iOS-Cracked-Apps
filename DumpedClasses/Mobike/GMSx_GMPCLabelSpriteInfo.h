//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "GMSx_PBGeneratedMessage.h"

@interface GMSx_GMPCLabelSpriteInfo : GMSx_PBGeneratedMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(nonatomic) unsigned long long glyphId; // @dynamic glyphId;
@property(nonatomic) _Bool hasGlyphId; // @dynamic hasGlyphId;
@property(nonatomic) _Bool hasHeight; // @dynamic hasHeight;
@property(nonatomic) _Bool hasWidth; // @dynamic hasWidth;
@property(nonatomic) _Bool hasX; // @dynamic hasX;
@property(nonatomic) _Bool hasY; // @dynamic hasY;
@property(nonatomic) int height; // @dynamic height;
@property(nonatomic) int width; // @dynamic width;
@property(nonatomic) int x; // @dynamic x;
@property(nonatomic) int y; // @dynamic y;

@end

