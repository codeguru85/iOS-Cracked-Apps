//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UITableViewCell.h"

@class FeedbackMessage, NSDateFormatter, NSMutableArray, SDKAttributedLabel, UILabel, UIView;

@interface FeedbackListViewCell : UITableViewCell
{
    FeedbackMessage *_message;
    unsigned long long _style;
    unsigned long long _backgroundStyle;
    SDKAttributedLabel *_labelText;
    id <FeedbackListViewCellDelegate> _delegate;
    NSDateFormatter *_dateFormatter;
    NSDateFormatter *_timeFormatter;
    NSDateFormatter *_fullFormatter;
    UILabel *_labelTitle;
    NSMutableArray *_attachmentViews;
    UIView *_accessoryBackgroundView;
    id _updateAttachmentNotification;
}

+ (double)heightForTextInRowWithMessage:(id)arg1 tableViewWidth:(double)arg2;
+ (double)heightForRowWithMessage:(id)arg1 tableViewWidth:(double)arg2;
@property(retain, nonatomic) id updateAttachmentNotification; // @synthesize updateAttachmentNotification=_updateAttachmentNotification;
@property(retain, nonatomic) UIView *accessoryBackgroundView; // @synthesize accessoryBackgroundView=_accessoryBackgroundView;
@property(retain, nonatomic) NSMutableArray *attachmentViews; // @synthesize attachmentViews=_attachmentViews;
@property(retain, nonatomic) UILabel *labelTitle; // @synthesize labelTitle=_labelTitle;
@property(retain, nonatomic) NSDateFormatter *fullFormatter; // @synthesize fullFormatter=_fullFormatter;
@property(retain, nonatomic) NSDateFormatter *timeFormatter; // @synthesize timeFormatter=_timeFormatter;
@property(retain, nonatomic) NSDateFormatter *dateFormatter; // @synthesize dateFormatter=_dateFormatter;
@property(nonatomic) __weak id <FeedbackListViewCellDelegate> delegate; // @synthesize delegate=_delegate;
@property(retain, nonatomic) SDKAttributedLabel *labelText; // @synthesize labelText=_labelText;
@property(nonatomic) unsigned long long backgroundStyle; // @synthesize backgroundStyle=_backgroundStyle;
@property(nonatomic) unsigned long long style; // @synthesize style=_style;
@property(retain, nonatomic) FeedbackMessage *message; // @synthesize message=_message;
- (void).cxx_destruct;
- (void)imageButtonPressed:(id)arg1;
- (void)layoutSubviews;
- (void)setAttachments:(id)arg1;
- (_Bool)isSameDayWithDate1:(id)arg1 date2:(id)arg2;
- (id)backgroundColor;
- (void)updateAttachmentFromNotification:(id)arg1;
- (void)unregisterObservers;
- (void)registerObservers;
- (void)dealloc;
- (id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2;

@end

