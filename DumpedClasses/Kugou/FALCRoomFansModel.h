//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "FAModel.h"

@class NSString;

@interface FALCRoomFansModel : FAModel
{
    unsigned long long _senderId;
    unsigned long long _total;
    unsigned long long _roomId;
    NSString *_nickName;
    NSString *_userLogo;
    unsigned long long _richLevel;
    unsigned long long _coin;
    unsigned long long _kugouId;
    unsigned long long _starLevel;
    unsigned long long _userId;
    long long _contributionRank;
}

@property(nonatomic) long long contributionRank; // @synthesize contributionRank=_contributionRank;
@property(nonatomic) unsigned long long userId; // @synthesize userId=_userId;
@property(nonatomic) unsigned long long starLevel; // @synthesize starLevel=_starLevel;
@property(nonatomic) unsigned long long kugouId; // @synthesize kugouId=_kugouId;
@property(nonatomic) unsigned long long coin; // @synthesize coin=_coin;
@property(nonatomic) unsigned long long richLevel; // @synthesize richLevel=_richLevel;
@property(retain, nonatomic) NSString *userLogo; // @synthesize userLogo=_userLogo;
@property(retain, nonatomic) NSString *nickName; // @synthesize nickName=_nickName;
@property(nonatomic) unsigned long long roomId; // @synthesize roomId=_roomId;
@property(nonatomic) unsigned long long total; // @synthesize total=_total;
@property(nonatomic) unsigned long long senderId; // @synthesize senderId=_senderId;
- (void).cxx_destruct;

@end

