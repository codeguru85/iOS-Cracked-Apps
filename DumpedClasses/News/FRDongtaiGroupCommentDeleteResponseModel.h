//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "TTResponseModel.h"

@class FRDeleteCommentDataStructModel, NSString<Optional>;

@interface FRDongtaiGroupCommentDeleteResponseModel : TTResponseModel
{
    NSString<Optional> *_message;
    FRDeleteCommentDataStructModel *_data;
}

@property(retain, nonatomic) FRDeleteCommentDataStructModel *data; // @synthesize data=_data;
@property(retain, nonatomic) NSString<Optional> *message; // @synthesize message=_message;
- (void).cxx_destruct;
- (void)reset;
- (id)init;

@end

