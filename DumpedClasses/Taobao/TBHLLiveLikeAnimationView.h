//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

@class TBHLLiveStarAnimation, UILabel;

@interface TBHLLiveLikeAnimationView : UIView
{
    _Bool _likeLabelHidden;
    UILabel *_likeLabel;
    TBHLLiveStarAnimation *_starAnimationView;
    double _animationHeight;
}

+ (id)animationViewWithFrame:(struct CGRect)arg1;
+ (id)likeViewWithAnimationHeight:(double)arg1;
@property(nonatomic) _Bool likeLabelHidden; // @synthesize likeLabelHidden=_likeLabelHidden;
@property(nonatomic) double animationHeight; // @synthesize animationHeight=_animationHeight;
@property(retain, nonatomic) TBHLLiveStarAnimation *starAnimationView; // @synthesize starAnimationView=_starAnimationView;
@property(retain, nonatomic) UILabel *likeLabel; // @synthesize likeLabel=_likeLabel;
- (void).cxx_destruct;
- (void)startLikeAnimation;
- (void)setupSubviews;
- (void)setFrame:(struct CGRect)arg1;
- (id)initWithFrame:(struct CGRect)arg1 hiddenLikeLabel:(_Bool)arg2;

@end

